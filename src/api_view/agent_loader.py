...
def query_display_messages(collection, thread_id: str) -> Optional[List[Dict[str, Any]]]:
    """查询展示消息（唯一共享 loader，方案 6.1）。

    session_display_messages 文档结构：{"thread_id", "index", "message": {...}}。
    Web 进程（AgentLoader.get_display_messages）与 standalone worker
    （agent.memory.worker_runner）必须使用同一提取契约 [doc["message"] ...]，
    避免两边漂移导致 worker 找不到 user/assistant message id。
    """
...
class AgentLoader:
    ...
    async def initialize(self):
        ...
        try:
            ...
            # 3. 预计算 MCP 工具 + 图表工具 + SubAgent YAML 配置
    ...
    async def get_display_messages(self, thread_id: str) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            db = self._mongodb_client[MONGODB_DB_NAME]
            collection = db["session_display_messages"]
            messages = query_display_messages(collection, thread_id)
            if messages is None:
                return None
