...
class ProcurementContext:
    ...
    # Memory v2（方案 18.6）：仅供本次 invocation 使用，绝不写入 state/checkpoint
    invocation_id: str = ""     # 服务端每次 API/Command(resume) 调用生成
    resume_id: Optional[str] = None  # HITL 恢复的可信 resume_id（服务端签发）
    memory_snapshot: Any = None      # MemoryInvocationSnapshot | None
...
class ChatRequest(BaseModel):
    ...
    user_id: str = Field("Quan", description="用户唯一标识")
