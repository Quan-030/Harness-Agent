...
from agent.config import LOCAL_SKILLS_DIR, SANDBOX_SKILLS_ROOT, SYSTEM_SKILLS_STORE_NAMESPACE


class SkillsSyncMiddleware(AgentMiddleware):
    """技能文件同步中间件，依赖沙箱后端的文件操作能力。"""

    def __init__(self, backend: BaseSandbox, store=None) -> None:
        super().__init__()
    ...
    def before_agent(self, state: Dict[str, Any], runtime: Any) -> Optional[Dict[str, Any]]:
        ...
        if new_skills:
            if self.store is not None:
                self._persist_system_skills(new_skills)
    ...
    async def abefore_agent(self, state: Dict[str, Any], runtime: Any) -> Optional[Dict[str, Any]]:
        ...
        if new_skills:
            if self.store is not None:
                await loop.run_in_executor(None, self._persist_system_skills, new_skills)
            return self._make_notification(new_skills)
        return None
    ...
    def _persist_system_skills(self, skill_names: List[str]) -> None:
        """将已同步的本地技能文件持久化到 StoreBackend 系统命名空间。

        key 格式: /{scope}/{skill_name}/...，与 UserSkillsRestoreMiddleware
        恢复时的路径映射一致。
        """
        ...
        for skill_name in skill_names:
            ...
            for local_file in skill_dir.rglob("*"):
                if not local_file.is_file():
                    continue
                relative_path = local_file.relative_to(local_skills_dir).as_posix()
                store_key = f"/{relative_path}"
                content_str = local_file.read_text(encoding="utf-8")
                try:
                    self.store.put(
