...
② 持久化到 StoreBackend（跨会话保留）：
   系统预置技能（src/skills/ 下已有）：assign_skill 自动检测并跳过持久化。
   系统技能已存在于共享命名空间 ("system_skills",)，所有用户可读，无需重复存储。
   用户自创技能：assign_skill 自动持久化到用户隔离命名空间 ("user_skills", user_id)。
   下次会话 UserSkillsRestoreMiddleware 自动恢复到沙箱 /skills/{scope}/{name}/。

③ 清理: execute("rm -rf /skills/main/{name}")
```

> scope 由用户指定的目标子 Agent 决定（查 Scope 映射表）。如用户未指定，先提醒再执行。
> 系统技能仅执行沙箱 scope 复制，不重复持久化；用户自创技能正常持久化到用户命名空间。
...
- `read_file` / `write_file`: 文件读写（沙箱 + StoreBackend）
- `ls`: 查看目录结构
