 ...
 * @param {string|null} resumeId - 中断时服务端签发的可信 resume_id（随 interrupt 事件下发）
 ...
 * @returns {Promise} 返回包含 thread_id, content, tool_calls 的结果
 */
export async function resumeChat(threadId, resumeData, resumeId = null, callbacks = {}, signal = null) {
  let fullContent = ''
  let toolCalls = []
  let toolStack = []
  ...
  try {
    const response = await fetch(`${API_BASE}/${threadId}/resume`, {
      ...
      body: JSON.stringify({ resume: resumeData, resume_id: resumeId }),
      signal: signal
    })
